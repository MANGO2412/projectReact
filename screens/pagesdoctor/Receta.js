...
const Receta = () => {
  ...
  const getFile=async()=>{
    try {
       ...
       console.log(resp.data[0]['patient_details'])
       console.log(resp)
      setFile(resp.data)
    } catch (error) {
      console.log(error)
 ...
 const clear=async()=>{
  await SecureStore.deleteItemAsync('fileID')
 }
  //get file
  useFocusEffect(
    useCallback(() => {
  ...
  const sendFormData = async () => {
    try {
      ...
      let  id=await SecureStore.getItemAsync('fileID')
      const response = await fetch('https://apifullheath.onrender.com/files/edit/' + id, {
        method: 'POST',
        headers: {
        ...
        >
                ...
                {file[0]?(
                      ...
                      <View>
                        <View style={{ marginBottom: 8, display: 'flex', flexDirection: 'row' }}>
                          ...
                          <Text style={{ marginTop: 0, fontSize: 16 }}>{' '+ file[0]["patient_details"].name + ' ' + file[0]["patient_details"].lastname}</Text>
                          {/* <Text style={{ marginTop: 0, fontSize: 16 }}>{file["patient_details"].lastname}</Text> */}
                        </View>
                        ...
                        <View style={{ marginBottom: 8, marginBottom: 12 }}>
                          ...
                          <View style={{ marginBottom: 8 }}>
                            ...
                            <TextInput
                              ...
                              onChangeText={(value) => inputHandle('medication', value)}
                              style={{
                                borderWidth: 1,
                                borderColor: '#CBD5E0',
                                paddingVertical: 8,
          ...
          {/* <View>
            ...
            <View>
              <View style={{ marginBottom: 8, display: 'flex', flexDirection: 'row' }}>
                <Text style={{ marginTop: 0, fontSize: 16, fontWeight: 'bold', marginBottom: 5 }}>Nombre:</Text>
                <Text style={{ marginTop: 0, fontSize: 16 }}>{file["patient_details"].}</Text>
              </View>
              <View style={{ marginBottom: 14, display: 'flex', flexDirection: 'row' }}>
                <Text style={{ marginTop: 0, fontSize: 16, fontWeight: 'bold' }}>Edad:</Text>
                <Text style={{ marginTop: 0, fontSize: 16 }}>  {file["patient_details"].age}  </Text>
                <Text style={{ marginTop: 0, fontSize: 16, fontWeight: 'bold' }}>  Peso:</Text>
                <Text style={{ marginTop: 0, fontSize: 16 }}> {file["patient_details"].weight}  30kg </Text>
              </View>
              <View style={{ marginBottom: 8, display: 'flex', flexDirection: 'row', backgroundColor: '#39a969', width: '100%', height: 30 }}>
                <Text style={{ marginTop: 7, marginLeft: 70, fontSize: 16, fontWeight: 'bold', marginBottom: 5, color: '#fff' }}>Diagnostico y tratamiento</Text>
              </View>
